    ...
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="ASN名称" prop="asnName">
         <el-input v-model="searchInfo.asnName" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="定时任务ID" prop="scheduleTaskId">
            
             <el-input v-model.number="searchInfo.scheduleTaskId" placeholder="搜索条件" />

        </el-form-item>
           <el-form-item label="任务状态" prop="histStatus">
            <el-select v-model="searchInfo.histStatus" clearable placeholder="请选择" @clear="()=>{searchInfo.histStatus=undefined}">
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form class="row-form" :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="定时任务ID:"  prop="scheduleTaskId" >
              <el-input v-model.number="formData.scheduleTaskId" :clearable="true" placeholder="请输入定时任务ID" />
            </el-form-item>
            ...
            <el-form-item label="耗时:"  prop="costTime" >
              <el-input v-model.number="formData.costTime" :clearable="true" placeholder="请输入耗时" />
            </el-form-item>
            <el-form-item label="任务状态:"  prop="histStatus" >
            ...
            <el-form-item class="full-width" label="任务结果:"  prop="taskResult" >
              <el-input :rows="25" type="textarea" v-model="formData.taskResult" :clearable="true"  placeholder="请输入任务结果" />
            </el-form-item>
          </el-form>
    </el-drawer>
  </div>
...
<script setup>
import {
  createScheduleTaskHist,
  deleteScheduleTaskHist,
  deleteScheduleTaskHistByIds,
  findScheduleTaskHist,
  getScheduleTaskHistList,
  updateScheduleTaskHist
} from '@/api/cfscan/scheduleTaskHist'
...
const getTableData = async() => {
  const table = await getScheduleTaskHistList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteScheduleTaskHistFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteScheduleTaskHistByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateScheduleTaskHistFunc = async(row) => {
    const res = await findScheduleTaskHist({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteScheduleTaskHistFunc = async (row) => {
    const res = await deleteScheduleTaskHist({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
