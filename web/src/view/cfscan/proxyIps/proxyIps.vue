  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
          <template #default="scope">
          <el-button type="primary" link icon="edit" class="table-button" @click="updateProxyIpsFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getProxyIpsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteProxyIpsByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateProxyIpsFunc = async(row) => {
    const res = await findProxyIps({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteProxyIpsFunc = async (row) => {
    const res = await deleteProxyIps({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
